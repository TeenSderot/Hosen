  ...
  <View style={styles.svgWrapper}>
    <Svg viewBox="0 0 280 350" width="100%" height={400} style={{marginLeft:50,marginTop:50}}>
      ...
      {fingersData.map((finger) => {
        let scaleY = fingersState[finger.name] ? 0.25 : 1;
        let scaleX = 1;
        let translateY = fingersState[finger.name] ? (1 - scaleY) * finger.height : 0;
        let translateX = 0;

        if (finger.name === "thumb") {
          scaleY = fingersState.thumb ? 0.7 : 1;
          scaleX = fingersState.thumb ? 0.8 : 1;
          translateY = fingersState.thumb ? (1 - scaleY) * finger.height : 0;
          translateX = fingersState.thumb ? (1 - scaleX) * finger.height * 0.3 : 0;
        }
        ...
        return (
          <G
            ...
            scaleY={scaleY}
            scaleX={scaleX}
            originX={finger.origin.x}
            originY={finger.origin.y}
            translateY={translateY}
            translateX={translateX}
            onPress={() => toggleFinger(finger.name)}
          >
            <Path d={finger.d} fill={defaultColor} />
          </G>
        );
      })}
    </Svg>
...
const styles = StyleSheet.create({
  container: {
    flexGrow: 1,
    alignItems: "center", // מרכז את תוכן ה-ScrollView
  },
  svgWrapper: {
    width: "100%", // חייב כדי שה-Svg יתפרס
    alignItems: "center", // מרכז את ה-Svg בתוך ה-View
  },
});
